...
output_raster <- "input/data/HabitatMap_it3.tif"
...
burn_raster <- rast("input/data/forest_data/forest_mask_it3.tif")
...
cat("Forest remasking done: HabitatMap_it3.tif mask.\n")
...
raster_path <- "input/data/HabitatMap_it3.tif"

polygon_path <- "input/data/mapsheet_grid.gpkg"

output_dir <- "input/data/habitat_map_maptiles/maptiles_it3"
...
cat("Done clipping HabitatMap_it3.tif to maptiles extents.\n")
...
new_mask_path <- file.path("input", "data", "habitat_map_maptiles", "maptiles_it3")
existing_tiles_path <- file.path("input", "model_data", "tiles")

split_dirs <- c("training", "validation", "test")

output_base <- file.path("input", "model_data", "masks_it3")
...
for (new_mask_file in new_mask_files) {
  ...
  for (split_dir in split_dirs) {
    ...
    for (tile in existing_tile_files) {
      ...
      out_file <- file.path(
        out_dir,
        paste0("mask_it3_", row, "_", col, "_", mapnr, "_", region, ".tif")
      )
...
cat("\n=====================================\n")
cat("Done clipping masks to tiles for iteration 3.\n")
cat("=====================================\n")